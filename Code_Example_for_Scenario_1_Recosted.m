...
% This code is based on the data given for Scenario 1 (re-costed economic
...
f1 = figure(1);
hold on
plot(pareto_Risk(5:12), pareto_ROI(5:12), '-or', 'LineWidth', 2,'MarkerSize',5);
...
f3 = figure(3);
b = bar(pareto_w(5:12,1:N), 'stacked');
threshold = 0.05; % 5% threshold
normalized_data = pareto_w(5:12, 1:N) ./ sum(pareto_w(5:12, 1:N), 2);
for i = 1:q
    cum_val = 0;
    for j = 1:N
...
xticks(1:8);
xticklabels(compose('%.1f', pareto_w(5:12,N+1)));
light_green = [0.7 1 0.7]; 
light_green_2 = [0.4 1 0.4];
light_red = [1 0.5 0.5];
